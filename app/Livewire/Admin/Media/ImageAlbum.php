...
abstract class ImageAlbum extends Component
{
    use WithFileUploads;
    ...
    abstract protected function mediaManagerClass(): string;

    abstract protected function storeUploadedImage(object $mediaManager, UploadedFile $upload, string $defaultText): void;

    abstract protected function uploadRoute(): string;
    ...
    public function uploadImages(): void
    {
        ...
        $mediaManager = app($this->mediaManagerClass());

        foreach ($this->uploads as $upload) {
            $this->storeUploadedImage($mediaManager, $upload, $this->defaultTextForUpload($upload));
        }
    ...
    {
        app($this->mediaManagerClass())->deleteMedia($this->findImage($imageId), auth()->user());
    ...
    public function moveImage(int $targetImageId, ?int $draggedImageId = null, string $position = 'before'): void
    {
        ...
        app($this->mediaManagerClass())->updateSortOrder($this->imageModelClass(), $ids, auth()->user());
    ...
    {
        return view('livewire.admin.media.image-album', [
            'albumClass' => $this->albumClass(),
            'imageUploadRoute' => $this->uploadRoute(),
            'images' => $this->images(),
    ...
    public function syncImageForms(): void
    {
        $this->imageForms = $this->images()
            ->mapWithKeys(fn (Model $image): array => [
                $image->id => [
                    'caption' => $image->getAttribute('caption'),
                    'alt_text' => $image->getAttribute('alt_text'),
                    'title_text' => $image->getAttribute('title_text'),
                    'description' => $image->getAttribute('description'),
                    'credit' => $image->getAttribute('credit'),
                    'is_decorative' => (bool) $image->getAttribute('is_decorative'),
                ],
            ])
            ->all();
    }

    private function findImage(int $imageId): Model
    {
        return $this->owner()
            ->images()
            ->whereKey($imageId)
    ...
    private function editingImage(): ?Model
    {
