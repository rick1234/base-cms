...
use App\Actions\Admin\Faq\UpsertFaqCategory;
...
class FaqCategoryController extends Controller
{
    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        return view('admin.faq.categories.index', [
            ...
            'pageName' => __('FAQ Categories'),
        ]);
    }

    public function store(FaqCategoryRequest $request, UpsertFaqCategory $upsert): RedirectResponse
    {
    ...
    public function edit(Request $request): View
    {
        ...
        return view('admin.faq.categories.edit', [
            ...
            'categoriesByParent' => $categories->groupBy(fn (FaqCategory $category): int => $category->parent_id ?: 0),
            'routeNames' => $this->routeNames(),
            'pageName' => __('Edit FAQ category'),
            'backUrl' => route($this->routeName('index'), array_filter(['categoryId' => $category?->id])),
        ]);
    }

    public function save(FaqCategoryRequest $request, UpsertFaqCategory $upsert): RedirectResponse
    {
        $category = $upsert->handle($request->validated(), $request->user(), $request->faqCategory());
    ...
    private function categoryFromRequest(Request $request): ?FaqCategory
    {
    ...
    private function moveCategory(int $categoryId, string $direction): void
    {
        ...
        $siblings = FaqCategory::query()
            ->where('parent_id', $category->parent_id)
            ->orderBy('sort_order')
            ->orderBy('id')
            ->get();

        $currentIndex = $siblings->search(fn (FaqCategory $sibling): bool => $sibling->id === $category->id);
    ...
    {
        return (request()->routeIs('cms.*') ? 'cms.faq.categories.' : 'admin.faq.categories.').$name;
    }
}
