...
use App\Actions\Admin\Locations\UpsertLocationCategory;
use App\Http\Controllers\Admin\Concerns\UsesEditViewForCreate;
use App\Http\Controllers\Controller;
use App\Http\Requests\Admin\Locations\LocationCategoryRequest;
...
class LocationCategoryController extends Controller
{
    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        return view('admin.locations.categories.index', [
            'routeNames' => $this->routeNames(),
            'pageName' => __('Location category overview'),
        ]);
    }

    public function store(LocationCategoryRequest $request, UpsertLocationCategory $upsert): RedirectResponse
    {
    ...
    public function edit(Request $request): View
    {
        ...
        return view('admin.locations.categories.edit', [
            'category' => $category ?? new LocationCategory([
                'parent_id' => $request->integer('parent') ?: null,
                'status' => 'active',
            ]),
            'categories' => $categories,
            'categoriesByParent' => $categories->groupBy(fn (LocationCategory $category): int => $category->parent_id ?: 0),
            'routeNames' => $this->routeNames(),
            'pageName' => __('Edit location category'),
            'backUrl' => route($this->routeName('index'), array_filter(['categoryId' => $category?->id])),
        ]);
    }

    public function save(LocationCategoryRequest $request, UpsertLocationCategory $upsert): RedirectResponse
    {
    ...
    public function destroy(LocationCategory $locationCategory): RedirectResponse
    {
    ...
    private function categoryFromRequest(Request $request): ?LocationCategory
    {
        $id = (int) ($request->route('id') ?: $request->integer('id'));

        return $id > 0 ? LocationCategory::query()->findOrFail($id) : null;
    }
    ...
    private function moveCategory(int $categoryId, string $direction): void
    {
        ...
        $category = LocationCategory::query()->find($categoryId);

        if (! $category) {
            return;
        }

        $siblings = LocationCategory::query()
            ->where('parent_id', $category->parent_id)
            ->orderBy('sort_order')
            ->orderBy('id')
            ->get();

        $currentIndex = $siblings->search(fn (LocationCategory $sibling): bool => $sibling->id === $category->id);
    ...
    {
        return (request()->routeIs('cms.*') ? 'cms.locations.categories.' : 'admin.locations.categories.').$name;
    }
}
